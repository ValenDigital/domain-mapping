 ...
 * @since 4.2.0
 */
class Domainmap_Reseller_ResellerClub extends Domainmap_Reseller {

    const RESELLER_ID = 'ResellerClub';

    const COMMAND_CHECK_LOGIN_CREDS  = 'country/list.xml';
    ...
    const ACTION_CHECK_CLIENT_LOGIN      = 'dm_resellerclub_validate_client_login';
    const ACTION_ORDER_DOMAIN            = 'dm_resellerclub_order_domain';
    const ACTION_REGISTER_CLIENT         = 'dm_resellerclub_register_client';
    ...
    public function get_option( $name, $default = false ){
        ...
        return isset( $options[self::RESELLER_ID][$name] ) ?  $options[self::RESELLER_ID][$name] : $default;
    }
     ...
     */
    public function exec_command( $command, $arguments = array() ){
        $options = Domainmap_Plugin::instance()->get_options();

        $args = array();
        $args['auth-userid']    = $this->get_option( 'reseller_id' );
        $args['api-key']        = $this->get_option( 'api_key' );
        $api_url                = $this->_get_api_url() . $command;
        $args                   = array_merge( $args, $arguments );

        $api_url = esc_url_raw( add_query_arg( $args, $api_url ) );

        if( !($args['api-key']   && $args['auth-userid'] && $api_url)  ) return;
        ...
        $response_body = strpos($headers['content-type'], "json") !== false ?  json_decode( wp_remote_retrieve_body( $response ) ) : simplexml_load_string( wp_remote_retrieve_body( $response ) );
        ...
        if ( $response_code  === 200 ){
            ...
            if( strpos($headers['content-type'], "json") !== false ){
                ...
                $messages = isset( $response_body->message ) ?  (array) $response_body->message : false ;
                $error->add( $response_code, $messages? $messages[0] : __("Unknown", domain_map::Text_Domain) );
            }
            return $error;
        }
    }
    ...
    private function _exec_command( $command, $arguments = array() ) {
        return $this->exec_command( $command, $arguments );
    }
    ...
    public function save_options( &$options ) {
        ...
        // reseller id
        $id = trim( filter_input( INPUT_POST, 'map_reseller_id' ) );
        $id_needs_health_check = !isset( $options[self::RESELLER_ID]['reseller_id'] ) || $options[self::RESELLER_ID]['reseller_id'] != $id;
        $options[self::RESELLER_ID]['reseller_id'] = $id;

        // user name
        $api_key = trim( filter_input( INPUT_POST, 'map_reseller_api_key' ) );
        $api_needs_health_check = !isset( $options[self::RESELLER_ID]['api_key'] ) || $options[self::RESELLER_ID]['api_key'] != $api_key;
        $options[self::RESELLER_ID]['api_key'] = $api_key;

        $options[self::RESELLER_ID]['environment'] = trim( filter_input( INPUT_POST, 'map_reseller_resellerclub_environment' ) );


        // payment gateway
        $gateway = filter_input( INPUT_POST, 'map_reseller_resellerclub_payment' );
        if ( $gateway !== false ) {
        ...
        // client registration
        $options[self::RESELLER_ID]['enable_registration'] = (bool) filter_input( INPUT_POST, 'map_reseller_resellerclub_client_registration' );

        $needs_health_check = $api_needs_health_check && $id_needs_health_check;


        // validate credentials
        $options[self::RESELLER_ID]['valid'] = $needs_health_check || ( isset( $options[self::RESELLER_ID]['valid'] ) && $options[self::RESELLER_ID]['valid'] === false )
    ...
    /**
     * Returns api url based on api mode
     *
     * @return string
     */
    private function _get_api_url(){
        return $this->get_environment() === "test" ? "https://test.httpapi.com/api/" : 'https://httpapi.com/api/';
    }
    ...
    private function _validate_credentials() {
        $object = $this->_exec_command( self::COMMAND_CHECK_LOGIN_CREDS );
        $this->_log_api_request(self::REQUEST_VALIDATE_CREDENTIALS, $object);
        $valid = is_object( $object ) && !is_wp_error( $object );

        $transient = self::RESELLER_ID . '_errors_' . get_current_user_id();
    ...
    public function render_options() {
        ...
        $template = new Domainmap_Render_Reseller_ResellerClub_Settings( $options );

        $template->gateways = $this->_get_gateways();
        $template->gateway = $this->get_gateway( $options );
        $template->errors = get_site_transient( self::RESELLER_ID . '_errors_' . get_current_user_id() );
    ...
    protected function _check_domain( $tld, $sld ) {
        ...
        $this->_log_api_request( self::REQUEST_CHECK_DOMAIN, $json );
    ...
    public function purchase() {
        ...
        $this->_log_api_request( self::REQUEST_PURCHASE_DOMAIN, $response );
    ...
    private function _populate_dns_records( $tld, $sld ) {
        ...
        if ( !empty( $args ) ) {
            ...
            $this->_log_api_request( self::REQUEST_SET_DNS_RECORDS, $response );
        }
    }
...
//            "promocode" => $promocode,
    ...
    public static function get_domain_pricing(){
        return self::instance()->get_option( 'tlds', array() );
    }
     ...
     */
    private function _log_api_request( $type, $object ) {
        if ( !is_object( $object ) ) {
            return;
        }
